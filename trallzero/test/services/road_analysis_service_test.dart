...
void main() {
  group('RoadAnalysisService - Slope and Elevation Tests', () {
    ...
    test(
      'classifySlope correctly classifies both uphill and downhill slopes',
      () {
    ...
    test(
      'smoothElevations eliminates high-frequency DEM jitter on flat terrain',
      () {
        ...
        final smoothed = service.smoothElevations(
          points,
          rawElevations,
          windowMeters: 80.0,
        );
        ...
        for (var i = 0; i < smoothed.length - 1; i++) {
          ...
          expect(
            ...
            lessThan(0.8),
            reason:
                'Step diff between point $i and ${i + 1} should be smoothed',
          );
        }
      },
    );
